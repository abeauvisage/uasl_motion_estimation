...
struct FrameInfo{
    ...
    void write(cv::FileStorage& fs) const{
        fs  << "{" << "start" << fframe << "stop" << lframe << "rate" << skip
            << "bframe" << bias_frame << "initframe" << init << "}";
    }
...
struct TrackingInfo{
    ...
    void write(cv::FileStorage& fs) const{
        fs  << "{"
            << "feats" << nb_feats << "window" << window_size << "ba_rate"
            << ba_rate << "parallax" << parallax << "feat_cov" << feat_cov
            << "}";
    }
...
struct DatasetInfo{
    ...
    void write(cv::FileStorage& fs) const{
		fs	<< "{"
			<< "dir" << dir << "image_file" << image_filename << "gt_file"
            << gt_filename << "imu_file" << imu_filename << "video"
            << (is_video==true?"true":"false") << "gps" << gps_orientation
            << "type" << (type==SetupType::mono?"mono":"stereo")
			<< "scaled" << (scaled_traj?"true":"false") << "poses"
            << (poses==PoseType::ABSOLUTE?"absolute":"relative") << "camID" << cam_ID
            << "init_orientation" << q_init.getCoeffs() << "init_position" << p_init
			<< "cam_orientation" << q_cam_to_base.getCoeffs() << "cam_position"
            << p_cam_to_base
			<< "}";
    }
...
void loadImagesKitti(   const std::string& dir, int nb,
                        std::pair<cv::Mat,cv::Mat>& imgs, const int padding=8);
cv::Mat loadImageKitti( const std::string& dir, int cam_nb, int img_nb,
                        const int padding=8);
void loadImages(const std::string& dir, int nb, std::pair<cv::Mat,cv::Mat>& imgs,
                const int padding=5);
cv::Mat loadImage(const std::string& dir, int cam_nb, int img_nb, const int padding=5);
void loadPCImages(  const std::string& dir, int nb,
                    std::vector<std::pair<cv::Mat,cv::Mat>>& imgs, const int padding=5);
std::vector<cv::Mat> loadPCImage(   const std::string& dir, int cam_nb, int img_nb,
                                    const int padding=5);
...
struct ImageReader{
    ...
    ImageReader(const std::string& filename="", Type type_=Type::IMAGES): fimage{filename}, cap(dataset_info.type==SetupType::stereo?2:1),type{type_},img_nb{0},img_stamp{0}{
        ...
        if(type == Type::VIDEO){ // if video needs to open video streams
            for(uint i=0;i<cap.size();i++){
                cap[i].open(dataset_info.dir+"/cam"+std::to_string(i)+"_image.avi");
            }
        }
        do{
    ...
    void openReader(const std::string& filename=dataset_info.dir+"/image_data.csv", Type type_=Type::IMAGES){
        ...
        if(!fimage.is_open())
            std::cerr << "[ImageReader] warning: could not find an image file in " << dataset_info.dir << std::endl;
        else if(type == Type::VIDEO){ // if video needs to open video streams
            for(uint i=0;i<cap.size();i++){
                cap[i].open(dataset_info.dir+"/cam"+std::to_string(i)+"_image.avi");
            }
        }
    ...
    std::pair<cv::Mat,cv::Mat> readStereo(){
      ...
      if(type==Type::IMAGES)
        ...
        if(!cap[0].isOpened() || !cap[1].isOpened()){
            std::cout << "[ImageReader] Warning: video stream is not opened!"<< std::endl;
            return imgs;
        }
