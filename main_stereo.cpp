...
/**	To run the example:
	cd build/
	./stereo_example ../examples/stereo/data
**/
int main(int argc, char** argv){
    ...
    if(imgs.first.empty())
        return -1;
    else if(st==stereo && imgs.second.empty())
        return -1;*/
    ...
    StereoVisualOdometry stereo_viso(param_stereo);
    ...
    for(int i=fi.fframe+fi.skip;i<fi.lframe;i+=fi.skip){
        ...
        vector<StereoOdoMatches<Point2f>> savedMatches;
		...
		if(matchesFile.isOpened()){
        	...
        	for(it_matches=matches.begin();it_matches!=it_matches_end;it_matches++){
            	savedMatches.push_back(StereoOdoMatches<Point2f>(Point2f((float)(*it_matches)["f1x"],(*it_matches)["f1y"]),Point2f((float)(*it_matches)["f2x"],(float)(*it_matches)["f2y"]),Point2f((float)(*it_matches)["f3x"],(float)(*it_matches)["f3y"]),Point2f((float)(*it_matches)["f4x"],(float)(*it_matches)["f4y"])));
        	}
        }
	...
	// uncomment to read images
        ...
        stereo_viso.process(savedMatches);
        Tr = stereo_viso.getMotion();
